...
function initTimeline(data, years) {
	...
	const gLines = g.append('g').attr('id', 'gLines').attr('visibility', 'visible')

	d3.select('#timeline-top').selectAll('input').on('change', function() {
		if (this.value == 'bars') {
	...
	function mouseover(d) {
		...
		tooltip.select(".tooltipCircle").style("fill", codeColour(d.code)).attr("r", 5);
