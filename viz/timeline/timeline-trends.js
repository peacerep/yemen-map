/*
Trends Timeline (agreements grouped by year)
...
// Obtain data
var data = d3.csv("PAX_with_additional.csv")
...
function callFunction(data, code){
          ...
          for (year = 0; year < yrList.length; year++){
            ...
            var rects = chartGroup.selectAll("rects.agt")
                .data((years[year].values).filter(function(d){ return d[code] > 0; }))
              .enter().append("rect")
                .attr("class","agt")
